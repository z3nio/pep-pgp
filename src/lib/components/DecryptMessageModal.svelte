<script lang="ts">
	...
	// Local
	export let decryptedMessage: string | undefined = undefined;
		...
		{:else if $modalStore[0].meta.hasError === false}
			<label class="label">
				<div class="flex gap-2 items-center"> 
					<span class="text-sm">Decrypted Message</span>
				</div>
				<textarea class="text-sm textarea h-44" rows="4" disabled>{$modalStore[0].meta.decryptedMessage}</textarea>
				<button class="btn btn-sm bg-primary-300 text-xs" use:clipboard={$modalStore[0].meta.decryptedMessage}>Copy</button>
			</label>
		{/if}
