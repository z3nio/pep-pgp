<script lang="ts">
	...
	// Local
	export let encryptedMessage: string | undefined = undefined;
	...
	<div class="p-4 space-y-4 shadow-xl modal-example-form card w-modal">
		...
		{:else if $modalStore[0].meta.hasError === false}
			<label class="label">
				...
				<textarea class="text-sm textarea" rows="4" disabled>{$modalStore[0].meta.encryptedMessage}</textarea>
				<button class="btn btn-sm bg-primary-300 text-xs" use:clipboard={$modalStore[0].meta.encryptedMessage}>Copy</button>
			</label>
		{/if}
